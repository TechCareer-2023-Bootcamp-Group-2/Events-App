...
const Header = () => {
  ...
  return (
    <header className="flex justify-between items-center px-[20px] md:px-[50px] min-h-[65px] border border-white border-opacity-40 bg-white bg-opacity-80 
      ...
      <div className="md:hidden">
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white mt-2 shadow-lg border border-white border-opacity-40 rounded-md">
            <Link to={"/"} className={`block py-2 px-4 no-underline hover:text-red-500 ${pathname === "/" && "text-purple-500"}`}>
              Home
            </Link>
